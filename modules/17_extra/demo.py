...
def experimento_modelo():
    ...
    total_int8 = sum(
        quantization_error(p.data)["quantized_bytes"] for p in matrices
...
def main() -> None:
    ...
    e_canal = [quantization_error(w.data, True)["relative_error"] * 100 for _, w in ejemplos]
    e_tensor = [quantization_error(w.data, False)["relative_error"] * 100 for _, w in ejemplos]
    x = range(len(nombres))
