...
overfit_single_batch = resolve("13_entrenamiento_final", "overfit_single_batch")
...
def experimento_entrenar(cfg_dev, cfg, dataset, get_batch):
    ...
    entrenador = Entrenador(cfg, modelo, get_batch, device=cfg_dev, on_sample=muestra)
    muestra(0)  # antes de entrenar nada
    estado = entrenador.entrenar(console=console)
...
def main() -> None:
    ...
    curvas = None
    if not solo_test:
        estado, entrenador = experimento_entrenar(cfg_dev, cfg, dataset, get_batch)
        from llmfs.train import EntrenamientoLogger

        curvas = EntrenamientoLogger(cfg.run_dir, reanudando=True).leer_csv()
        console.print(
    ...
    fig.tight_layout()
    destino = figures_dir() / "13_entrenamiento_final.png"
