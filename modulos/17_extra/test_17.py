...
def test_por_canal_es_mas_preciso_que_por_tensor():
    ...
    e_canal = ej.quantization_error(w, per_channel=True)["error_relativo"]
    e_tensor = ej.quantization_error(w, per_channel=False)["error_relativo"]
...
def test_tiene_todas_las_claves():
    ...
    for clave in ("error_relativo", "error_maximo", "error_medio", "compresion",
                  "bytes_original", "bytes_cuantizado"):
        assert clave in e, f"falta la clave {clave!r}"


def test_la_compresion_de_fp32_a_int8_es_4x():
    assert ej.quantization_error(torch.randn(4, 8))["compresion"] == 4.0
...
def test_el_error_relativo_de_pesos_reales_es_pequenyo():
    ...
    error = ej.quantization_error(w)["error_relativo"]
...
def test_los_bytes_cuantizados_incluyen_las_escalas():
    ...
    assert e["bytes_cuantizado"] > w.numel(), (
        "los bytes cuantizados tienen que incluir el vector de escalas"
    )
    assert e["bytes_original"] == w.numel() * 4
...
def test_el_error_relativo_es_independiente_de_la_escala():
    ...
    torch.manual_seed(0)
    w = torch.randn(8, 32)
    a = ej.quantization_error(w)["error_relativo"]
    b = ej.quantization_error(w * 1000)["error_relativo"]
...
def test_el_error_del_modelo_completo_es_asumible():
    ...
    errores = [
        ej.quantization_error(p.data)["error_relativo"]
