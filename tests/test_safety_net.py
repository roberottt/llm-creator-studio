...
def test_todos_los_ejercicios_declarados_existen_en_ejercicios_py():
    """Si `curriculum.py` declara un ejercicio, `exercises.py` tiene que definirlo."""
    ...
    for module in modulos_escritos():
        ...
        if cargado is None:
            faltan.append(f"{module.id}/exercises.py no importable")
            continue
        for ex in module.exercises:
...
def test_la_teoria_sigue_la_estructura_pedagogica():
    """Cada THEORY.md cierra reconociendo lo que no esta cerrado, y enlaza al glosario."""
    problemas: list[str] = []
    for module in modulos_escritos():
...
def test_la_teoria_tiene_cuerpo_suficiente():
    ...
    assert not cortas, f"THEORY.md demasiado corta (minimo 900 palabras): {cortas}"
...
def test_cada_solucion_incluye_el_codigo_completo():
    ...
    Cada SOLUTION.md termina con una seccion de codigo completo y copiable de todos los
...
def test_el_codigo_de_cada_solucion_se_puede_copiar_y_pegar():
    ...
    Se extrae el bloque de codigo de cada SOLUTION.md y se comprueba que:
      1. es python valido,
      2. define todos los ejercicios del modulo,
      3. no usa ningun nombre que no este disponible en el `exercises.py` del alumno
    ...
    for module in all_modules():
        ...
        # Que nombres tiene disponibles el alumno en su exercises.py
